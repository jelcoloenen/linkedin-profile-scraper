"""Process LinkedIn profile URLs from a CSV file using the MCP server."""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('profile_processing.log')
    ]
)
...
async def process_profiles_from_csv(
    ...
    """
    logger.info("=" * 80)
    logger.info("LinkedIn Profile Processor - Starting")
    logger.info("=" * 80)
    ...
    def progress_callback(current, total, profile_data):
        ...
        if profile_data and profile_data.get("success"):
            try:
                ...
                if current == 1:
                    import json
                    with open('/tmp/profile_raw_debug.txt', 'w', encoding='utf-8') as f:
                        f.write(f"URL: {profile_data.get('url')}\n")
                        f.write(f"Type: {type(profile_data.get('raw_data'))}\n")
                        f.write(f"Length: {len(str(profile_data.get('raw_data')))}\n")
                        f.write(f"\n========== RAW DATA ==========\n")
                        f.write(str(profile_data.get('raw_data')))
                    logger.info("DEBUG: Saved first profile's raw data to /tmp/profile_raw_debug.txt")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description='Process LinkedIn profile URLs from CSV using MCP server'
    )
    ...
    parser.add_argument(
        '--delay',
        default='3-5',
        help='Delay between requests in seconds (default: 3-5)'
    )
    ...
    if '-' in args.delay:
        ...
        min_delay = max_delay = float(args.delay)
